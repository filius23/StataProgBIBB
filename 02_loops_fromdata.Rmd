...
foreach lvl  of global ausb {
	...
	mat GX = `lvl', r(p25), r(mean), r(p50), r(p75) // Werte zeilenweise verbinden
	mat G2[`lvl',1] = GX
}
mat l G2
```
...
+ einfach einige globals geben 
+ dann zählen und vllt `if`
...
- Kopieren Sie die oben erstellte Schleife und passen Sie diese so an, dass jeweils die Werte für Min, Mean, Median, Max und N in `matrix` gesammelt werden. 
...
- Verwenden Sie die Schleife aus [Übung 2](#retue2) und passen diese so an, dass die Labels für `gkpol` erhalten werden legen Sie die Ergebnismatrix in einen `frame` ab.
...
- Erstellen Sie nun eine Schleife um die existierende Schleife herum, sodass nicht nur die Werte entlang von `gkpol`, sondern auch `F233_nuts2` berechnet und gesammelt werden. Erstellen Sie also einen "nested loop", sodass erst eine `matrix` für `gkpol` und dann eine für `F233_nuts2` erstellt wird.

- Wie würden Sie die Schleife anpassen, wenn nur Gruppen mit N >= 100 ausgewertet werden dürften?
